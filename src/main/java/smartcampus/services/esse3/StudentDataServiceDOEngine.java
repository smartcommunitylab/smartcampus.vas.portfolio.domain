...
public class StudentDataServiceDOEngine extends AbstractDOEngineImpl {

    public StudentDataServiceDOEngine() {
        super();
    }

    private static String[] actions = new String[]{
            "_serviceNotification_updateStudentData",
    };
    ...
    protected Object executeAction(String actionName, DomainObjectWrapper obj, Tuple t, Set<DomainEvent> outEvents, Set<EvaluableDomainOperation> ops, String securityToken, String bundleId) throws DomainDataHandlerException {
        if ("_serviceNotification_updateStudentData".equals(actionName)) {
            return _serviceNotification_updateStudentData(t, obj, outEvents, ops, securityToken, bundleId);
        }
        return null;
    }

    private Object _serviceNotification_updateStudentData(Tuple tuple, DomainObjectWrapper obj, Set<DomainEvent> evts, Set<EvaluableDomainOperation> ops, String securityToken, String bundleId) throws DomainDataHandlerException {
        	 try {
		Tuple body = new Tuple();
		body.putAll((Map)new smartcampus.services.esse3.data.StudentDataNotificationConverter().fromMessage((java.io.Serializable)tuple.get("data")));
		getDomainObjectHandler().publishCustomEvent("updateStudentData", body, obj, evts, bundleId);
	} catch (Exception e) {
...
return null;
    ...
    protected ActionInvoke ruleApplies(EvaluableDomainSubscriptionRule rule, DomainObjectWrapper obj, String bundleId) throws DomainDataHandlerException {
        if ("_subscribe_serviceNotification_updateStudentData".equals(rule.getRule().getEngineName())) {
            Tuple payload = (Tuple) rule.getEvent().getPayload();
            if (true){
                Tuple tuple = new Tuple();
                tuple.put("data",payload.get("data"));
                return new ActionInvoke("_serviceNotification_updateStudentData", tuple);
            }
        }
        return null;
    }
    ...
    public String getType() {
        return "smartcampus.services.esse3.StudentDataService";
    }
    ...
    public Collection<DomainSubscriptionRule> getSubscriptions(DomainObjectWrapper obj, String bundleId) throws DomainDataHandlerException {
        ...
        
        {
            ...
            rule.setName("_subscribe_serviceNotification_updateStudentData");
            rule.setSourceId(null);
            rule.setSourceType("smartcampus.service.esse3.GetStudentData");
            rule.setTargetId(obj.getId());
            rule.setTargetType(obj.getType());
            rule.setEngineName("_subscribe_serviceNotification_updateStudentData");
    ...
    public it.sayservice.platform.domain.model.interfaces.DomainTypeInterface getInterface() throws it.sayservice.platform.core.domain.common.exception.DomainDataHandlerException {
        return smartcampus.services.esse3 .StudentDataServiceInterface .getInstance();
    }
